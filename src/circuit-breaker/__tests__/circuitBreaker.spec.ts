...
describe('circuitBreaker', () => {
  ...
  it('should repeatedly switch to halfopen when requests keep failing', async () => {
    const breaker = circuitBreaker('abcd', { failRate: 75, cacheLifetime: 3600, switchToHalfOpenIn: 10, logger });
    breaker.clearCache();
    await breaker.request(async () => await failPromise());
    await breaker.request(async () => await failPromise());
    await breaker.request(async () => await failPromise());
  ...
  it('should recover back from failing state and reset statistics of a counter on success promise', async () => {
    const breaker = circuitBreaker('qwerty', { failRate: 75, cacheLifetime: 3600, switchToHalfOpenIn: 10, logger });
    breaker.clearCache();
    await breaker.request(async () => await failPromise());
    await breaker.request(async () => await failPromise());
    await breaker.request(async () => await failPromise());
